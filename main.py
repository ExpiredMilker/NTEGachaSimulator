...
class AnimatedBoardCanvas:
    ...
    def _generate_limited_board(self, board_id="limited_xun"):
        ...
        核心思路：
        1. 根据board_id从board_data.py获取对应棋盘的完整配置
        2. 提取该棋盘的所有格子实际坐标（用于视觉显示）
        3. 按编号顺序排列游戏逻辑索引（确保起点在index=0）
        4. 建立映射：游戏索引 → 硬编码坐标
        5. 动态构建类型映射表（根据棋盘配置中的角色名）
        ...
        try:
            ...
            print(f"[错误] 无法获取棋盘 {board_id} 的配置: {e}")
            print(f"[回退] 使用默认棋盘（浔）")
            config = board_data.get_board_config("limited_xun")
        
        self.board_name = config["display_name"]
        self.board_type = config["board_type"]
        self.current_board_id = board_id
        
        # [关键修复] 重置cell_size为默认值，避免使用之前自适应后的值导致坐标计算错误
        # 如果不重置，切换到新棋盘时会基于错误的cell_size计算坐标，导致显示异常
        self.cell_size = 32
        cs = self.cell_size
        ...
        def build_type_mapping():
            """
            动态构建类型映射表
            根据当前棋盘配置生成所有可能的格子名称到类型的映射
            
            映射规则：
            - "起点" -> start
            - "于此同行（*）" -> companion（无论括号内是什么角色名）
            - 固定名称 -> 对应固定类型
            """
            T = {
                "起点": "start",
        ...
        if not layout_data:
            print(f"[警告] 棋盘 {board_id} 没有布局数据，使用默认布局")
            layout_data = board_data.get_limited_board_layout()

        main_path_raw = layout_data["main_path"]   # [(row, col, name, idx), ...]
        branch1_raw = layout_data["branch1"]       # [(row, col, name, seq), ...]
        branch2_raw = layout_data["branch2"]       # [(row, col, name, seq), ...]

        print("=" * 80)
        print("开始生成限定棋盘布局（基于board_data.py硬编码数据）")
        print(f"主路径: {len(main_path_raw)}格, 分支1: {len(branch1_raw)}格, 分支2: {len(branch2_raw)}格")
        print("=" * 80)

        # 输出解析的主路径格子
        for row, col, name, idx in main_path_raw:
            x = col * cs + cs // 2
            y = row * cs
            print(f"[主路径{idx:2d}] ({row:2d},{col:2d}) {name} -> ({x}, {y})")

        # 输出分支格子
        for i, (row, col, name, seq) in enumerate(branch1_raw):
            x = col * cs + cs // 2
            y = row * cs
            print(f"[分支1-{i}]   ({row:2d},{col:2d}) {name} (原序号{seq}) -> ({x}, {y})")
        for i, (row, col, name, seq) in enumerate(branch2_raw):
            x = col * cs + cs // 2
            y = row * cs
            print(f"[分支2-{i}]   ({row:2d},{col:2d}) {name} (原序号{seq}) -> ({x}, {y})")

        print("\n" + "=" * 80)
        print(f"数据加载完成！主路径: {len(main_path_raw)}格, 分支1: {len(branch1_raw)}格, 分支2: {len(branch2_raw)}格")
        print("=" * 80)
        ...
        # 创建格子编号映射数组（用于调试输出）
        # 主路径：使用原始编号（0-54）
        # 分支：使用 B1-X 或 B2-X 格式（X为0-based索引）
        self.cell_numbers = []
        ...
        branches_template = board_data.LIMITED_BOARD_TEMPLATE.get("branches", {})
        
        s_character_branch = branches_template.get("s_character_zone", {})
        skin_surprise_branch = branches_template.get("skin_surprise", {})
        
        self.branch_entries = {
            "s_character": {
                "name": f"S级角色区({s_char_name})",
                "entry_main_idx": s_character_branch.get("entry_main_idx", 16),
                "skip_main_idx": s_character_branch.get("skip_main_idx", 17),
                "exit_main_idx": s_character_branch.get("exit_main_idx", 18),
                "branch_start": b1_start,
                "branch_len": b1_count,
                "s_pos_in_branch": 0,     # S级角色在分支第0位
            },
            "skin_surprise": {
                "name": "皮肤惊喜区",
                "entry_main_idx": skin_surprise_branch.get("entry_main_idx", 43),
                "skip_main_idx": skin_surprise_branch.get("skip_main_idx", 44),
                "exit_main_idx": skin_surprise_branch.get("exit_main_idx", 45),
        ...
        print(f"[棋盘信息] 同行角色: {list(companions.values())}")
...
"""
        ...
        def run_n(idx):
            ...
            try:
                if idx >= 10:
                    ...
                    if actual_rolls != 10:
                        print(f"[调试-10连-错误] ⚠️ 投掷次数异常! 实际={actual_rolls}, 期望=10")
                ...
                print(f"\n{'='*80}")
                print(f"[10连抽] 开始第{idx+1}/10次投掷 (总次数: {self.engine.state.total_rolls})")
                print(f"{'='*80}")
                ...
                if is_already_in_branch and target_branch_data:
                    ...
                    target_global_pos = self.board_canvas._cycle_pos(global_pos, dv)
                    print(f"[主路径移动-10连] {global_pos} → {target_global_pos}")
                ...
                # [调试-10连] 记录执行后的状态
                print(f"[调试-10连-后] 第{idx+1}次执行后: total_rolls={self.engine.state.total_rolls}, s_pity={self.engine.state.s_pity_counter}, gift={self.engine.state.gift_counter}")
                print(f"[调试-10连-后] 第{idx+1}次奖励数: {len(result.get('rewards', []))}, pity_event={result.get('pity_event')}")
                ...
                def after_one():
                    try:
                        ...
                        try:
                            ...
                            print(f"[错误-致命] 恢复失败: {e2}，尝试跳过本次并继续")
    ...
    def _on_reset(self):
        ...
        if messagebox.askyesno(
            ...
            # [修复] 获取当前棋盘ID并传递给GameEngine，确保奖励池保持一致
