...
class LimitedCaeusBoard(BaseBoard):
    """限定棋盘（卡厄斯）"""
    ...
    def get_s_pool(self) -> list:
        ...
        - 硬保底：90次必得S级
        """
        return [{"name": "卡厄斯", "rate": 0.0187, "limited": True}]
    ...
    def get_variant_transforms(self) -> Dict[int, Dict[str, str]]:
        ...
        2. 勇者宝箱变（12个）→ 必定获得S级角色(100%)
           ...
           分支B1: B1-2, B1-6, B1-8 (3个)
           注：B1-4不是变格（与浔不同！）
        """
        return {
            # ===== 学徒宝箱变（5个）=====
            # CSV中明确带"变"字的学徒宝箱
            # 注意：35号不是变格（浔是变格，卡厄斯不是）
            ...
            # 分支3个（CSV确认：分支3/7/9为勇者宝箱变）
            # 注意：B1-4(59号)不是变格（浔是变格，卡厄斯不是）
    ...
    def generate_layout(self) -> Tuple[List[Tuple[int, int]], List[str], List[str], Dict]:
        ...
        cell_positions = [
            ...
            (6, 21),   # B1-0 于此同行（卡厄斯）[S级角色] 入口
            (6, 22),   # B1-1 改装时刻·涂装
            (6, 23),   # B1-2 勇者宝箱变
            (6, 24),   # B1-3 学徒宝箱
            (7, 24),   # B1-4 勇者宝箱（注意：不是变格！）
            (8, 24),   # B1-5 学徒宝箱
            (8, 23),   # B1-6 勇者宝箱变
            (8, 22),   # B1-7 学徒宝箱
            (8, 21),   # B1-8 勇者宝箱变 出口
        ...
        cell_types = [
            ...
            "companion",                # 44 于此同行（海月）
            "mist_box",                 # 45 迷迭棋盒（30个白色棋子）
            ...
            "brave_chest",              # 54 勇者宝箱变

            # ===== 分支1（9格）：B1-0 ~ B1-8 =====
            "companion",                # B1-0 于此同行（卡厄斯）[S级角色]
            "vehicle_paint",            # B1-1 改装时刻·涂装
            "brave_chest",              # B1-2 勇者宝箱变
            "apprentice_chest",         # B1-3 学徒宝箱
            "brave_chest",              # B1-4 勇者宝箱（非变格）
            "apprentice_chest",         # B1-5 学徒宝箱
            "brave_chest",              # B1-6 勇者宝箱变
            "apprentice_chest",         # B1-7 学徒宝箱
            "brave_chest",              # B1-8 勇者宝箱变
        ...
        branch_entries = {
            "B1": {
                # 分支1入口=16号格子(6,21)学徒宝箱，出口=18号格子(8,21)勇者宝箱
                "start_main_idx": 16,
                "branch_cells": [
                    # 分支1共9格（S级角色区）
                    ("companion", "于此同行（卡厄斯）"),   # B1-0 入口(S级角色)
                    ("glider_skin", "改装时刻·涂装"),       # B1-1
                    ("brave_chest", "勇者宝箱变"),           # B1-2
                    ("apprentice_chest", "学徒宝箱"),        # B1-3
                    ("brave_chest", "勇者宝箱"),             # B1-4 （注意：不是变格！）
                    ("apprentice_chest", "学徒宝箱"),        # B1-5
                    ("brave_chest", "勇者宝箱变"),           # B1-6
                    ("apprentice_chest", "学徒宝箱"),        # B1-7
                    ("brave_chest", "勇者宝箱变"),           # B1-8 出口
                ],
                "rejoin_main_idx": 18,
            },
            "B2": {
                ...
                "branch_cells": [
                    ...
                    ("brave_chest", "勇者宝箱"),             # B2-5
                    ("brave_chest", "勇者宝箱"),             # B2-6
                    ("brave_chest", "勇者宝箱"),             # B2-7
