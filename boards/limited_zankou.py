...
class LimitedZankouBoard(BaseBoard):
    """限定棋盘（残红）"""
    ...
    def get_s_pool(self) -> list:
        ...
        - 硬保底：90次必得S级
        """
        return [{"name": "残红", "rate": 0.0187, "limited": True}]
    ...
    def generate_layout(self) -> Tuple[List[Tuple[int, int]], List[str], List[str], Dict]:
        ...
        数据来源：限定棋盘残红地图手动统计.csv
        布局结构：蛇形路径 + 2个分支（B1、B2）
        坐标系统与浔完全相同（残红与浔共享同一套地图布局）
        ...
        branch_entries = {
            "B1": {
                ...
                "start_main_idx": 16,
                "branch_cells": [
                    ("companion", "于此同行（残红）"),   # B1-0 入口(S级角色)
